...
The server may still return **HTML** for human flows (e.g. opening the **confirm** or **unsubscribe** link from email, and the static thank-you page at `GET /api/confirm/thanks`). Those are UX helpers, not part of the Swagger contract. For the documented JSON `{"ok":true}` response on success, call `GET /api/confirm/{token}?format=json` or `GET /api/unsubscribe/{token}?format=json`.
...
| `WEB_UI_URL`                                                            | Subscribe SPA URL (e.g. Vercel)                 | Used for HTML confirm/unsubscribe flows (links back to the subscribe UI). If unset, links fall back to the documented Vercel demo URL; set explicitly in production.                 |
